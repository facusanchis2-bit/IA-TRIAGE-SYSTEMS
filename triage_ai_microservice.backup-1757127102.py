...
# --------- App meta ---------
APP_VERSION = "1.0.0-P1-nlp-uncertainty-panel-name"
...
def init_db():
    ...
    with sqlite3.connect(DB_PATH) as con:
        ...
        # columnas que pueden faltar en bases viejas
        needed_cols = [
            ("created_at", "TEXT"),
            ("chief_complaint", "TEXT"),
            ("override_reason", "TEXT"),
...
def save_session_to_db(state: Dict[str, Any]) -> None:
    ...
    patient_name = (state.get("patient",{}) or {}).get("nombre_apellido") or ""
    with sqlite3.connect(DB_PATH) as con:
        con.execute("""
...
def build_steps(path:str)->List[str]:
    return [
        ...
        "dirigidas",  # sólo si la rama tiene followups
        "consentimiento",
    ]

# --------- NLP: spaCy opcional + reglas ---------
_SPACY = None
def _maybe_load_spacy():
    global _SPACY
    if _SPACY is not None: return _SPACY
    try:
        import spacy
        # es_core_news_md si está instalado; si no, intentará el pequeño; si nada, fallback
        for m in ["es_core_news_md","es_core_news_sm"]:
            try:
                _SPACY = spacy.load(m)
                break
            except Exception:
...
_MEDICATION_WORDS = {"ibuprofeno","paracetamol","amoxicilina","omeprazol","aspirina","salbutamol","metformina","enalapril"}
_LOC_WORDS = {"pecho","panza","abdomen","cabeza","espalda","garganta","pierna","brazo","cintura","urinaria","orinar"}
_COMORB = {"diabetes":"E11","hipertensión":"I10","presión alta":"I10","asma":"J45","epoc":"J44","cardiopatía":"I51"}
_SYMPTOMS_CODES = {
    "dolor de pecho":("R07.4","SNOMED:29857009"),
    "falta de aire":("R06.0","SNOMED:267036007"),
    "tos":("R05","SNOMED:49727002"),
    "vómitos":("R11","SNOMED:422400008"),
    "fiebre":("R50.9","SNOMED:386661006"),
    "dolor abdominal":("R10.4","SNOMED:21522001"),
    "dolor de cabeza":("R51","SNOMED:25064002"),
}

_DUR_RE = re.compile(r"(hace\s+)?(\d+)\s*(hora|horas|día|días|semana|semanas)", re.I)
...
def _parse_duration(text:str) -> Optional[Dict[str,Any]]:
    ...
    hours = n * (1 if unit.startswith("hora") else (24 if unit.startswith("día") else 24*7))
    return {"value":n, "unit":unit, "hours":hours}

def medical_ner(text:str) -> Dict[str,Any]:
    """Devuelve entidades y mapeo mínimo a CIE/SNOMED (demo)."""
    txt = normalize(text)
    nlp = _maybe_load_spacy()
    meds=set(); loc=set(); toks=[]
    if nlp:
        doc = nlp(txt)
        for t in doc:
            toks.append(t.text)
            w = t.lemma_.lower()
            if w in _MEDICATION_WORDS: meds.add(w)
            if w in _LOC_WORDS: loc.add(w)
    else:
        toks = txt.split()
        meds = {w for w in toks if w in _MEDICATION_WORDS}
        loc  = {w for w in toks if w in _LOC_WORDS}

    # síntomas comunes por substring
    symptoms=set()
    for k in _SYMPTOMS_CODES:
        if k in txt: symptoms.add(k)

    dur = _parse_duration(txt)
    codes = { k: {"ICD10":_SYMPTOMS_CODES[k][0], "SNOMED":_SYMPTOMS_CODES[k][1]} for k in symptoms }
...
# --------- Features & Clasificación con incertidumbre ---------
_WEIGHTS={
...
def simple_ner_and_features(ans:Dict[str,str])->Dict[str,Any]:
    ...
    txt_all = " ".join(normalize(ans.get(k,"")) for k in fields)
    # NER más rico sobre motivo + asociados + dirigidas
    ner = medical_ner(" ".join([ans.get("motivo",""), ans.get("sintomas_asoc",""), ans.get("dirigidas","")]))

    feat={
        "chest_pain": int(any(k in txt_all for k in ["pecho","precordial"])),
        "dyspnea": int(any(k in txt_all for k in ["falta de aire","ahogo"])),
        "palpitations": int(any(k in txt_all for k in ["latidos fuertes","palpitaciones"])),
        "syncope": int(any(k in txt_all for k in ["desmayo","se desmayó","sincope"])),
        "radiation_left_arm": int(any(k in txt_all for k in ["brazo izquierdo","mandíbula"])),
        "diaphoresis": int(any(k in txt_all for k in ["sudor","transpiración fría"])),
        "nausea": int(any(k in txt_all for k in ["náusea","nausea","vómit","vomit"])),
        "sudden_onset": int(any(k in txt_all for k in ["inicio súbito","de repente","brusco"])),

        "altered_consciousness": int(any(k in txt_all for k in ["inconsciente","no responde","confuso","somnoliento"])),
        "focal_deficit_sudden": int(any(k in txt_all for k in ["cara caída","hablar mal","debilidad del brazo","no mueve un lado","dificultad para hablar"])),
        "massive_bleeding": int(any(k in txt_all for k in ["sangrado abundante","hemorragia masiva"])),

        "hypotension": int("presión muy baja" in txt_all or "hipotens" in txt_all),
        "hemodynamic_instability": int("shock" in txt_all or "muy pálido" in txt_all),

        "diabetes": int("diabetes" in txt_all),
        "htn": int("presión alta" in txt_all or "hipertens" in txt_all),
    }
    # vitales numéricos
    ...
    entities={
        "symptoms_model": ner.get("symptoms",[]),
        "codes": ner.get("codes",{}),
        "medications": ner.get("medications",[]),
        "locations": ner.get("locations",[]),
        "duration": ner.get("duration",None),
        "tokens": ner.get("tokens",[]),
        "comorbidities":[k for k in _COMORB if k in txt_all],
...
def hard_overrides(feat:Dict[str,Any])->Optional[Dict[str,Any]]:
    ...
    if feat.get("altered_consciousness") and feat.get("focal_deficit_sudden"): return {"esi":1,"reason":"Alteración de conciencia + signos focales (ACV)"}
    ...
    # Sepsis (demo muy simple): fiebre alta + taquicardia + hipotensión textual
    if (feat.get("temp",0)>=39.5) and (feat.get("hr",0)>=120) and (feat.get("hypotension",0)==1):
...
def risk_score(f:Dict[str,Any])->float:
    ...
    # vitales (si no disparan override)
    ...
    # duración corta puede elevar (inicio súbito)
    if f.get("sudden_onset"): s += 0.6
    return s

_THRESHOLDS = {2:6.5, 3:3.5, 4:1.5}  # > T2 => 2; > T3 => 3; > T4 => 4; else 5
def predict_esi(f:Dict[str,Any])->Tuple[int,Optional[Dict[str,Any]],List[Dict[str,Any]],Dict[str,Any]]:
    ...
    # incertidumbre: cerca del umbral (|margin|<0.3) -> subir una categoría (más urgente)
    low_conf = abs(margin) < 0.3
    if low_conf and esi<5:
        esi = max(2, esi) - 1 if esi in (3,4,5) else esi  # subir a más urgente  (3->2, 4->3)
        note="Borde de umbral: elevación de seguridad"
    else:
        note=""
    ...
    return esi,None,shap[:10],{"low_confidence":low_conf, "note":note, "score":round(s,2)}
...
def list_sessions(
    ...
    if q:
        where.append("(session_id LIKE ? OR chief_complaint LIKE ? OR IFNULL(patient_name,'') LIKE ?)")
        args += [f"%{q}%", f"%{q}%", f"%{q}%"]
    if name:
        where.append("IFNULL(patient_name,'') LIKE ?"); args.append(f"%{name}%")
    if from_:
    ...
    with sqlite3.connect(DB_PATH) as con:
        ...
        rows=con.execute(f"""
          SELECT session_id, created_at, patient_name, chief_complaint,
                 predicted_esi, mapped_manchester, recommendation,
...
def export_excel(auth: bool = Depends(require_admin)):
    with sqlite3.connect(DB_PATH) as con:
        df=pd.read_sql_query("""
          SELECT session_id as Sesion, created_at as Fecha, patient_name as Paciente, chief_complaint as Motivo,
...
<table id="t"><thead><tr>
<th>Fecha</th><th>Sesión</th><th>Paciente</th><th>Motivo</th><th>ESI</th><th>Manchester</th><th>Recomendación</th><th>Override</th>
